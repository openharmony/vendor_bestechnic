...
static void testFileCreatAndClose(const char *file)
{
    ...
    int wLen = write(fd,file,strlen(file));
    LOG_I("fd:%d write file %s total bytes: %d\r\n", fd, file, wLen);
    close(fd);
    ...
    if (rlen <= 0) {
        LOG_E("read file %s total bytes: %d failed \r\n", file, rlen);
    } else {
        LOG_I("read file '%s' total bytes: %d %s \r\n", file, rlen,buf);
    }
...
void fs_test(void)
{
    ...
    testFileCreatAndClose("/data/testfile101");
}
