...
TARGET_PARAM=
XTS="all"
WIFIIOT_OUTFILE=wifiiot_bes2600w.bin
...
build()
{
  out_dir="${BASE_HOME}/out/bes2600w/xts_demo"
...
build_wifiiot()
{
    ...
    echo -e "xts_root_dir:${xts_root_dir} suite_out_zip:${suite_out_zip}"
    mkdir -p $DIST_DIR
    ...
    do
      ...
      python build.py -p xts_demo@bestechnic -f --patch --test xts ${element}
      suite_build_target=`echo "${element}" | awk -F "[/:]" '{print $NF}'`
      echo -e "suite_build_target:${suite_build_target}"

      echo -e "suite_out_dir:${suite_out_dir}"
      module_list_file=$suite_out_dir/module_info.json
      echo -e "module_list_file:${module_list_file}"
      ...
      echo -e "suite_module_name:${suite_module_name}"
      ...
      cp -f ${BASE_HOME}/out/bes2600w/xts_demo/unstripped/bin/${WIFIIOT_OUTFILE} ${suite_out_dir}/${subsystem_name}/${suite_module_name}.bin
